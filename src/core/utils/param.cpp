...
/* METAHEURISTIC */
int Parameters::tries = 1000000000;
int Parameters::max_insert = 20;
int Parameters::lazy_iterations_swap_ratio = 2000000;
int Parameters::execution_time = 1;
int Parameters::sleep = 0;
int Parameters::candidate_skip = 10;
int Parameters::lazy_tries = 50000000;
int Parameters::tabu_size = 50;
...
void Parameters::readParameters(std::string param_path) {
    ...
    if (f) {
        ...
        tries = 1000000000
        max_insert = 20
        lazy_iterations_swap_ratio = 2000000
        execution_time = 1
        sleep = 0
        candidate_skip = 10
        lazy_tries = 50000000
        ...
        */
        while (f >> command >> separator >> value) {
            if (command == "verbosity") {
            ...
            } else if (command == "tries")
                tries = stoi(value);
            else if (command == "max_insert")
                max_insert = stoi(value);
            else if (command == "lazy_iterations_swap_ratio")
                lazy_iterations_swap_ratio = stoi(value);
                ...
                lazy_tries = stoi(value);
            else if (command == "tabu_size")
                tabu_size = stoi(value);
        ...
        std::cout << "Warning: No param file found!" << std::endl;
